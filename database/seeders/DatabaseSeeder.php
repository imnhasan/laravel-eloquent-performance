...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        User::find(1)->update(['name' => 'Akash']);
        User::find(2)->update(['name' => 'Karim']);
        User::find(3)->update(['name' => 'Jamal']);
        User::find(4)->update(['name' => 'Hasan']);
        User::find(5)->update(['name' => 'Jonny']);
        User::find(6)->update(['name' => 'Joy']);

        $books->each(fn ($book) => Book::create($book));
